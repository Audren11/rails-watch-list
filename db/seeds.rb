...
debbieocean = Movie.create(title: "Ocean's Eight", overview: "Debbie Ocean, a criminal mastermind, gathers a crew of female thieves to pull off the heist of the century.", poster_url: "https://image.tmdb.org/t/p/original/MvYpKlpFukTivnlBhizGbkAe3v.jpg", rating: 7.0)

horror = List.create(name: "Horror")
americain = List.create(name: "Americain")
dramas = List.create(name: "Dramas")
action = List.create(name: "Action")

Bookmark.create(comment: "cool movie", list_id: action.id, movie_id: wonder_woman.id)
Bookmark.create(comment: "cool movie", list_id: dramas.id, movie_id: titanic.id)
